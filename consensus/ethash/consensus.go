...
var (
	...
	YapstoneBlockReward    *big.Int = big.NewInt(0)     // Block reward in wei for successfully mining a block upward from Yapstone
...
func (ethash *Ethash) Prepare(chain consensus.ChainReader, header *types.Header) error {
	if chain.GetHeaderByNumber(0).Hash() == params.YapstoneGenesisHash {
		// setup accumulateRewards for Yapstone
		accumulateRewards = yapstoneAccumulateRewards
	}
...
// yapstoneAccumulateRewards()
func yapstoneAccumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header, uncles []*types.Header) {
	// Select the correct block reward based on chain progression
	blockReward := YapstoneBlockReward
